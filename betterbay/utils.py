...
def create_timeslot_table(
    ...
    * ``proxy_class`` - a wrapper class for accessing an ``Occurrence`` object.
    ...
    #start_time = start_time.replace(tzinfo=pytz.timezone('US/Eastern')) if not start_time.tzinfo else start_time
...
#    n = time_zone.localize(dtstart)
    ...
    for item in sorted(items):
        if item.end_time.replace(tzinfo=None) <= dtstart:
            # this item began before the start of our schedle constraints
            continue

        if item.start_time.replace(tzinfo=None) > dtstart:
            rowkey = current = item.start_time.replace(tzinfo=None)
        else:
        ...
        while 1:
            ...
            if colkey not in timeslot:
                ...
                while current < item.end_time.replace(tzinfo=None):
                    rowkey = current
    ...
    table = []
    for rowkey, elem in timeslots.items():
        cols = empty_columns[:]
