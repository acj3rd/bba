...
def occurrence_view(
    ...
    if request.method == 'POST':
        if '_update' in request.POST:
            ...
            occurrence.delete()
            return http.HttpResponseRedirect('/bba/events')
...
def add_event(
    ...
    if request.method == 'POST':
        ...
        recurrence_form = recurrence_form_class(request.POST)
        print("Event valid: ", event_form.is_valid())
        print("Recurrence valid: ", recurrence_form.is_valid())
...
def day_view(request, year, month, day, template='daily_view.html', **params):
    ...
    """
    print('Dayview request for ', year, month, day)
